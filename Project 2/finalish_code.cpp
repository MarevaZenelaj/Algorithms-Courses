...
void heap_insert_employee(Employees heap_array[], Employees key, char parameter){
			...
			heap2_size++;
			heap_array[heap_size].total = -9999999999;
...
int main(){
	...
	if (file){
		while (!file.eof()){
			...
			heap_employees[heap_size] = added_employee;
		...
		if (n_file){
			while (!file.eof()){
				...
				if (position1>0){
					...
					heap_performance[position1].performance_score = 2 * heap_performance[position].total + heap_performance[position1].positive - heap_performance[position1].negative;
					...
					heap_insert_employee(heap_performance,new_emp);
					heap_insert_employee(heap_calls,new_emp);
					heap1_size++;
				}
			if (position2>0){
					...
					heap_calls[position2].negative = heap_employees[position].negative;
